...
export const getSortedProducts = (sort: 'asc' | 'desc') =>
  api.get<Product[]>(`/products?sort=${sort}`);
...
export const getSortedCarts = (sort: 'asc' | 'desc') =>
  api.get<Cart[]>(`/carts?sort=${sort}`);
export const addToCart = (cart: Omit<Cart, 'id'>) =>
  api.post<Cart>('/carts', cart);
export const updateCart = (id: number, cart: Partial<Cart>) =>
  api.put<Cart>(`/carts/${id}`, cart);
